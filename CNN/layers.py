...
'''
inputs:
  shape--- list [a,b], where a is the number of filters used in the Conv. layer and 
                       b is the number of elements each filter has (fan_in) 
  fan_in --- the number of elements each filter has.
  factor --- default value 2 if ReLU is used (He initialisation)

  output:
   randomly initialised weights of the filters of the layer  between -2 and +2 
   (number filters * fan_in) with mean=0 and standard deviation=sigma.
'''
...
class Convolution(Layer):
  ...
  def __init__(self, input_layer, num_filters, kernel_size, name, padding='SAME',
    ...
    if padding == 'SAME':
      # with zero padding
      self.shape = (N, num_filters, H, W) # shape of the output after the convolution is performed to the input
    ...
    fan_in = C * kernel_size**2   # the total number of elements of a single kernel
  ...
  def forward(self, x):
    k = self.kernel_size
    self.x_cols = im2col_cython(x, k, k, self.pad, self.stride) # to convert the input tensor (image or feature map) to column vector
